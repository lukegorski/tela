/**
 * PII scrubbing helpers for Sentry on apps/admin. Shared across
 * client/server/edge runtimes because the same payload shapes flow
 * through every Sentry event regardless of where it's captured.
 *
 ...
 *   2. tRPC request bodies on the catch-all capability route + the
 *      admin SSE chat endpoint (cofounder queries through AdminAiChat
 *      may include user emails, free-form notes, or other PII).
 *
 * Authorization / Cookie headers are scrubbed by Sentry's defaults.
 *
 * The admin variant scrubs only two URL prefixes (vs apps/web's five)
 * — admin doesn't expose `/api/trpc/chat.*`, `/api/trpc/profile.*`, or
 * `/api/trpc/wardrobe.*`. Everything admin does goes through
 * `capability.execute`, and the chat stream lives at /admin/chat/stream.
 */
