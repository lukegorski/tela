...
- **Descoped from this project (Luke, 2026-07-13, v0 dogfood):** deeper sizing — Tier-1 flagged collection and everything past it — is a **separate future project**, no longer scheduled inside this one's v1. v0 ships **recipe v2.1** (one shared px-per-cm scale over the footprint table, so cross-role proportion holds by construction — `packages/types/src/builderRecipe.ts`) as this project's accepted end-state; founder verdict: proportions acceptable, not perfect.
...
**v1 — Render + premium scaffolding:** "See it on the model" → try-on; `combo_hash` cache (copy-on-hit, version salt); trailing-7-day quota in `tryon.generate`; `users.plan` + admin toggle (plan AND features editable from admin users page); AI gate + fake door; quota UI. (Tier-1 size-estimation collection moved out — separate sizing project, §3.)
...
4. **Shared-surface WHITELIST for build sessions** (the only pre-flip edits allowed to existing files): (a) the 14 locale dictionary files (new keys only); (b) the admin users page (plan/features toggles); (c) the enhancement flow's additive, fail-open cutout hook. Everything else: new files only. Any session needing an exception STOPS and asks Luke.
   **Exception on record (2026-07-14, commit `9fdf118` on `builder-v0`; blessed by Luke via merge approval 2026-07-14):** null-occasion tolerance on outfit surfaces (`lib/types.ts`, outfits page, OutfitHero/OutfitGridCell/OutfitCard/OutfitPiecesSheet). Manual outfits carry no context row, so `outfit.list` returns `occasion: null` — the page as written crashed on `occasion.toLowerCase()` the moment the first manual outfit existed, making §3's "appear in grid like AI outfits" unsatisfiable without touching these files. Behavior for AI outfits is unchanged (occasion chip simply doesn't render when null). Alternative if byte-identity is preferred: revert `9fdf118` and filter `source='manual'` out of `outfit.list` until the IA flip (manual outfits would then be invisible outside the builder).
