...
- **`admin.listUsers` correlated-subquery SQL bug — AdminAiChat user-lookup tool non-functional** — P1 (urgent; admin chat's "find user by email/handle" workflow is broken on main today).
  ...
  Symptom (observed in the chat panel 2026-05-22): cofounder asks "find the user with email luke@lukegorski.com" → AI invokes `admin.listUsers` → tool returns Failed query error → AI gracefully falls back to `admin.getDashboardStats` (success but doesn't have user-lookup data) → AI replies asking the user to provide the user ID directly. Result is degraded but not fatal — the chat doesn't crash, the cofounder just can't actually look up users by email. The admin `/admin/users` *page* works because it's a different code path (page fetches likely don't use the same broken capability, or use it with different arguments that happen to dodge the bug).
  `params: {}` in the error payload confirms the issue: the SQL was constructed with no bound parameters where the outer-row references should be. The Drizzle `sql\`...\`` template literal is interpolating the column reference as a bare quoted identifier rather than as a qualified outer-row column.
  **Fix path**: open `packages/capabilities/src/admin/listUsers.ts` (or wherever the admin user-list capability lives), find the 5 correlated subqueries, replace bare `${users.id}` (or `${id}`) interpolations with either (a) Drizzle's column-ref helper that emits `"users"."id"`, or (b) explicit string `users.id` (un-quoted) so Postgres treats it as a column reference. Should be a 5-line fix once the right reference shape is found. Test by running the admin chat "find user by email" tool against any seeded user.
  **Why this surfaced now**: the AdminAiChat UI was newly available on main (merged with `feat/14c-ui` on 2026-05-22). This was probably the first time the user-lookup tool was exercised end-to-end against the deployed admin. Once the AdminAiChat capture-gap above is fixed, future regressions in admin tools will surface as Sentry events automatically.
