...
- **Admin `/admin/chat/stream` PII scrub validation post-14c-ui merge** — P3 (bound to `feat/14c-ui` merge).
  The admin sentry-scrub (`apps/admin/src/lib/sentry-scrub.ts`) is shipped and identical-in-shape to apps/web's (which is verified). The 2-path admin variant covers `/api/trpc/capability.execute` + `/admin/chat/stream`. Live end-to-end validation of the `/admin/chat/stream` scrub requires triggering a real error during a chat-stream SSE call carrying PII in the body — but the AdminAiChat UI surface (the Claude-icon panel that opens the chat input + calls `/admin/chat/stream`) lives in commit `f8f83a1` on branch `feat/14c-ui`, not on main. Once 14c-ui merges, open AdminAiChat → send a message containing an obvious PII string (e.g., `find user with email luke@lukegorski.com`) → force an error mid-stream (refresh during stream, or offline-toggle) → open the resulting Sentry event → confirm HTTP Request body is `[SCRUBBED]` (NOT the email). Until then, trust by parity — the scrub function is identical to web's working implementation modulo the path list.
  *Origin*: surfaced during apps/admin Sentry DoD verification 2026-05-22 — the live PII test was the one DoD bullet we could not check off because the chat UI isn't deployed.
