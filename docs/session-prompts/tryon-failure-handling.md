...
> - **Post-push verification (same day)**: Step 1 deployed and canary-verified. A $0-Fashn-spend forced failure (new job row against a real outfit, `model_image_url='not-a-url'`, enqueued with the pinned options) ran on the DEPLOYED worker: try_on_jobs row → `failed`/`cost_cents 0`, and the pgboss job row ended `state='failed', retry_limit=0, retry_count=0` with the pipeline error + dist stack as output — the old swallow would have left it `'completed'`, so this proves the re-throw chain live end-to-end. Canary row deleted after. Sentry-side sighting CONFIRMED by Luke in the UI 2026-07-05: both canaries landed in TELA-API-M as one issue (grouped by stack trace — 2 events, 1 user), first seen in release `d680c95d7b41` (2026-07-02 deploy), latest in `4cff930cbdea` (2026-07-05 deploy) — the release tags themselves double as proof both API deploys shipped. Capture-path detail: the capability `onError` hook in `apps/api/src/index.ts` captures first (tags `capability`/`source`/`requestId`); `worker.ts`'s capture of the same error object is deduped by the SDK, so events carry `capability: tryon.process` rather than the worker's `job` tag. Note for filtering: the Railway api service runs with `NODE_ENV` unset/`development`, so events are tagged environment=`development` despite being production traffic (and `tracesSampleRate` is 1.0 in this mode).
...
- [x] Step 1 merged: Sentry capture works, behavior unchanged, `retryLimit: 0` pinned. *(Pushed to main + deployed 2026-07-02 (`5d7f931`); forced-failure canary verified the re-throw chain live (pgboss `state='failed'`, `retry_limit 0`, `retry_count 0`, $0 spend). Sentry events confirmed in the UI 2026-07-05 — gate CLOSED. Follow-up `3dfb9d7` fixes `[object Object]` in failure messages, verified live by canary v2.)*
...
- [x] `docs/post-cutover-followups.md` "Try-on quality + failure handling" — failure-handling portion marked `[DONE]`. *(2026-07-05: observability portion DONE + verified; steps 2–4 recorded there as deferred by the 0%-failure-rate gate.)*
